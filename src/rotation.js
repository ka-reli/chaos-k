...
(function (root) {
  ...
  // Директива формы для модели. Форма — это ПРИЁМ ПОВЕСТВОВАНИЯ для
  // сюрреалистического эффекта (сам текст срывается в искажённую форму),
  // а НЕ предмет в мире. scope: 'whole' (весь ответ) | 'fragment' (отрезок).
  // mandatory: обязательно или «когда в сцене психодел».
  function formDirective(formRec, scope, mandatory) {
    if (!formRec || !formRec.instruction) return '';
    // Оформление навешивает расширение через CSS — модель не должна писать
    // HTML/CSS/код (иначе разметка протечёт в чат и в контекст).
    var noHtml = ' Пиши ПРОСТЫМ ТЕКСТОМ — без HTML, CSS, тегов <…> и блоков кода; оформление применится автоматически.';
    if (scope === 'fragment') {
      var lead = mandatory
        ? 'В этом ответе ОДИН отрезок повествования должен сорваться в сюрреалистическую форму.'
        : 'Если в сцене происходит психодел, бред или искажение реальности — поймай этот момент и сорви повествование в сюрреалистическую форму (если ничего такого нет — не вставляй).';
      return '〈Chaos-FX〉 ' + lead + ' Это ПРИЁМ НАРРАТИВА, а не предмет в мире. ' +
        'Оберни этот отрезок текста в [form:' + formRec.id + ']…[/form] и оформи саму прозу так: ' +
        formRec.instruction + ' Остальной текст — обычным повествованием.' + noHtml;
    }
    var leadW = mandatory
      ? 'ФОРМА ВСЕГО ОТВЕТА — оформи всё повествование так: '
      : 'Если сцена целиком уходит в психодел/бред — оформи всё повествование так: ';
    return '〈Chaos-FX〉 ' + leadW + formRec.instruction + noHtml;
  }

  // ── Сборка блока-подсказки для модели (RU — это контекст, не UI) ───────────
  // madness: режим «БЕЗУМИЕ» — размечать густо, вкладывать, не жалеть.
  function buildPrompt(effects, colors, form, scope, madness) {
    var lines = [];
    ...
    if (effects.length || colors.length) {
      if (madness) {
        lines.push('〈Chaos-FX〉 РЕЖИМ БЕЗУМИЯ. Размечай густо и безудержно: почти каждое предложение несёт эффект, длинные куски оборачивай целиком, вкладывай эффекты и цвета друг в друга, комбинируй несочетаемое. Чем психоделичнее, тем лучше. Метки пиши как есть, не объясняй их.');
      } else {
        lines.push('〈Chaos-FX〉 Расставляй метки прямо в тексте по смыслу. Оборачивай не только отдельные слова, но и ЦЕЛЫЕ ФРАЗЫ И ПРЕДЛОЖЕНИЯ, когда эффект относится ко всей мысли — длинный кусок под одной меткой выглядит лучше, чем дробление по словам. Метки пиши как есть, не объясняй их.');
      }
      ...
      if (effects.length) {
        ...
        lines.push('Эффекты — [fx:имя]слово или целая фраза[/fx]: ' + fx);
      }
      ...
      lines.push('Пример на фразе: [fx:' + ex1 + ']весь мир качнулся и поплыл в сторону[/fx], а потом ' +
        '[fx:' + ex2 + '][' + exC + ']тьма сомкнулась над её головой[/' + exC + '][/fx].');
      // Иначе модели с thinking-блоком начинают вслух рассуждать о разметке.
      lines.push('Не упоминай, не обсуждай и не планируй эти метки в размышлениях — просто расставь их в финальном тексте ответа.');
    }
  ...
  function rotate(settings) {
    ...
    return {
      ...
      colors: colors,
      form: form,
      prompt: buildPrompt(effects, colors, form, opts.formScope, opts.madness)
    };
  }
