...
// 9. pickForm возвращает форму из реестра, фильтр по настроениям работает.
var f9 = CFX.pickForm({ moods: ['nostalgia'], intensity: 10, rng: seeded(9) });
check('pickForm: форма из реестра', f9 && CFX.registry.form(f9.id) === f9, f9 && f9.id);
check('pickForm: фильтр по настроению', f9 && f9.moods.indexOf('nostalgia') !== -1, f9 && f9.moods.join(','));

// 10. pickForm уважает потолок накала (форма math intensity 7 не пройдёт при ceiling 4).
var seen = {};
for (var k = 0; k < 100; k++) {
  var fk = CFX.pickForm({ moods: [], intensity: 4, rng: seeded(200 + k) });
  if (fk) seen[fk.id] = (seen[fk.id] || 0) + 1;
}
var overC = Object.keys(seen).filter(function (id) { return CFX.registry.form(id).intensity > 4; });
check('pickForm: потолок накала', overC.length === 0, 'превышений: ' + overC.join(','));

// 11. formDirective: whole vs fragment, mandatory vs optional.
var dossier = CFX.registry.form('dossier');
